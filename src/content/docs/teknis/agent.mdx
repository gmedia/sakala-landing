...
<DiagramFigure
  caption="Arah komunikasi antara Console, API, dan Agent."
  alt="Console mengirim permintaan ke API. API menyimpan command. Agent menghubungi API untuk polling, claim, mengirim event dan log, lalu melaporkan hasil. Seluruh komunikasi dimulai dari Agent menuju API, tidak pernah sebaliknya. Agent kemudian menjalankan Docker, Railpack, dan Caddy pada runtime node."
  class="my-8"
>
  <SequenceFlow
    actors={["Console", "API", "Agent", "Runtime node"]}
    steps={[
      { from: "Console", to: "API", action: "meminta deployment" },
      { from: "API", to: "API", action: "menyimpan command berstatus pending" },
      { from: "Agent", to: "API", action: "polling command yang eligible" },
      { from: "Agent", to: "API", action: "claim atomik sebelum bekerja" },
      {
        from: "Agent",
        to: "Runtime node",
      ...
      },
      {
        from: "Agent",
        to: "API",
        action: "mengirim event dan log ter-redaksi",
      },
      { from: "Agent", to: "API", action: "melaporkan sukses atau gagal" },
    ]}
  />
</DiagramFigure>
